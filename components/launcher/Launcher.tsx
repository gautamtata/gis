	...
	useEffect(() => {
		const fetchKernelSpecs = async () => {
			try {
				...
				const kernelSpecs = await connectionManager.serviceManager?.kernelspecs
					.specs;

				if (kernelSpecs) {
					const newItems = Object.keys(kernelSpecs.kernelspecs)
						.filter((key) => kernelSpecs.kernelspecs[key]?.display_name)
						.map((key) => {
							return {
								...
								icon: kernelSpecs.kernelspecs[key]?.resources ? (
									<img
										...
										alt={kernelSpecs.kernelspecs[key]!.display_name}
										height='36px'
										width='36px'
									/>
								) : (
			...
			? [
					{
						...
						items: existingNotebooks.map((notebook) => ({
							...
							actionHandler: () => {
								useNotebookStore.getState().handleNotebookClick(notebook);
							},
						})),
					},
			  ]
